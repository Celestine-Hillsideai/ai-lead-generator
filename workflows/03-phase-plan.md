...
- [x] Real sourcing provider — 2026-09-14, **Apollo.io** (`lib/sourcing/apollo.ts`, `ApolloSourcingProvider`, Organization Search API `POST /api/v1/mixed_companies/search`); `tests/unit/apollo-provider.test.ts` covers request-mapping, response-mapping, employee-range parsing, and a non-ok-response failure path against a stubbed `fetch` — **not yet exercised against a live Apollo account/key**
...
- [ ] Real Apollo.io account/key exercised end-to-end (code is written and unit-tested against a stubbed response; `MOCK_SOURCING` stays the default until a real key is set)
