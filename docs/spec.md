...
- **Provider abstraction** (`CompanySourcingProvider`, `lib/sourcing/`), mirroring §14's decision-maker provider pattern: `findCompanies(query) -> SourcedCompanyCandidate[]`, where `query` is built entirely from the campaign's existing ICP fields (industry, geography, company size, target roles, offer description) — no separate ICP configuration is introduced. Every candidate carries a `sourceRef` pointing back to the provider's own result; a candidate's name/website must trace to that reference, never to a model's free-text output, so "never fabricate" (§4) is satisfied structurally rather than only by prompt instruction. `MOCK_SOURCING`/`SOURCING_API_KEY`/`SOURCING_PROVIDER` gate it: `MOCK_SOURCING` is the global kill switch exactly like `MOCK_EMAIL` gates real sending (mock is the default and only mode automated tests use); `SOURCING_PROVIDER` (default `"tavily"`) picks which real implementation the key belongs to. Two real providers exist:
  - **Tavily** (`lib/sourcing/tavily.ts`, `TavilySourcingProvider`, default) — a web-search API (`POST https://api.tavily.com/search`), two-stage. A single "companies in X industry" search only ever surfaces "Top N companies" listicle/directory pages, never individual homepages, and even a targeted "`<name>` official website" search frequently ranks LinkedIn/app-store/Wikipedia/Crunchbase above the real domain (confirmed live 2026-09-14 testing) — so: (1) a discovery search + AI extraction (`company_sourcing` agent type) identifies real company names actually named in the results; (2) per extracted name, a targeted resolution search (known non-company-owned domains excluded server-side) + a second AI call (`company_sourcing_resolution` agent type) picks which one (if any) of that company's own small result set is genuinely its official site. A company with no good resolution is dropped, never forced to a weak guess. Structurally fabrication-proof at both stages despite the AI steps: the model only ever picks a numeric `resultIndex` into real, already-fetched results (validated in-range — and, in stage 2, validated as belonging to that specific company — via a per-call `superRefine`, same idiom §16 uses for evidenceIds); a candidate's actual `website`/`sourceRef` is built from that real result, never from text the model writes. Chosen as the default because it has a genuinely usable free tier; confirmed live end-to-end 2026-09-14 (correctly resolved Paystack, Flutterwave, and Interswitch to their real domains against a Nigerian fintech ICP).
  - **Apollo.io** (`lib/sourcing/apollo.ts`, `ApolloSourcingProvider`) — its Organization Search API (`POST /api/v1/mixed_companies/search`), a licensed, already-structured company database (no AI step needed). Maps `industry` → `q_organization_keyword_tags`, `geography` → `organization_locations`, a parseable `companySize` range → `organization_num_employees_ranges`; `sourceRef` is `apollo://organizations/<id>`. Implemented and unit-tested but **not usable on Apollo's Free plan** — confirmed live via a `403 API_INACCESSIBLE` response 2026-09-14; available for whenever a paid Apollo plan is in use.
...
**Provider (`SearchProvider`, `lib/search/`)**: two real implementations, tried in order by `getSearchProvider()` (`FallbackSearchProvider`):

1. **Hunter.io** (`lib/search/hunter.ts`, `HunterSearchProvider`, primary, `HUNTER_API_KEY`) — Domain Search API returns already-structured real people for a domain (name, position, email, a per-address verification status), aggregated from Hunter's own licensed sources. No AI step at all: every field maps directly from Hunter's real response, so "never fabricate" holds trivially here, not just structurally. `verification.status` maps to this app's `verified`/`unverified`/`invalid`/`unknown` (`"valid"` → verified, `"accept_all"`/`"webmail"` → unverified, `"invalid"`/`"disposable"` → invalid). This is the only source of a genuinely *verified*, sendable email in the pipeline — added 2026-09-16 after live testing showed Tavily-only search almost never surfaces a real email (most companies simply don't publish one), which made sourced leads uncontactable, defeating the point of generating them.
2. **Tavily** (`lib/search/tavily.ts`, `TavilySearchProvider`, fallback, `SEARCH_API_KEY`) — used only when Hunter's domain search returns nothing (its database doesn't cover every company). One search (`"<company>" <role> OR founder OR "leadership team"`, boosted toward the company's own domain) plus one AI extraction call (`decision_maker_search` agent type, `prompts/decision-maker-search.prompt.ts`) surfaces real, named candidates, structurally fabrication-proof via the same `resultIndex`-into-real-results discipline as §11A's providers. Its own email guard (only kept if literally present in the result content, verified in code) still applies for this path, but in practice yields mostly `"unknown"` — this fallback exists mainly to at least get a real name for the salutation when Hunter has nothing.

Both feed `agents/decision-maker-agent.ts`'s own synthesis pass (which still also weighs the company's own crawled pages). That pass gets an additional code-level guard (`reconcileWithSearchResults`): any candidate matched back to a real `SearchProvider` record (by name or source URL) has its `email`/`emailStatus`/`sourceUrl` overwritten from that real record rather than trusted from the model's own copy — protects a genuinely verified email from being silently altered while the model transcribes it through its synthesis pass, which matters now that a real email actually gets used for sending (§20), not just displayed. Confirmed live 2026-09-14/16 (Tavily: correctly identified Paystack's/Flutterwave's/Interswitch's real founders/CEOs from real pages, no fabricated emails; Hunter: real verified emails returned for Paystack staff with per-address confidence and verification status).
